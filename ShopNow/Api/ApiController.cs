    ...
    public class ApiController : Controller
    {
        ...
        public JsonResult Register(CustomerCreateViewModel model)
        {
            ...
            catch (Exception ex)
            {
                ...
                var date = dateAndTime.ToString("d");
                var time = dateAndTime.ToString("HH:mm");
                Helpers.PushNotification.SendbydeviceId($"Hi, " + otpmodel.Otp + " is the OTP for (Shop Now Chat) Verification at " + time + " with " + otpmodel.ReferenceCode + " reference - Joyra", "Snowch", "","", customer.FcmTocken ?? "".ToString());
                otpmodel.Status = 0;
                otpmodel.DateEncoded = DateTime.Now;
                otpmodel.DateUpdated = DateTime.Now;
                db.OtpVerifications.Add(otpmodel);
        ...
        public JsonResult GetDeliveryBoyPayout(DateTime startDate, DateTime endDate, string phoneNo, int page = 1, int pageSize = 5)
        {
            ...
            .Select(i => new DelivaryBoyPayoutReportViewModel.PayoutOut
            {
                ...
                TotalAmount = i.FirstOrDefault().d.WorkType == 1 ? i.Sum(a => ((a.c.Distance <= 5) ? 20 : 20 + (a.c.IsPickupDrop == false ? ((a.c.Distance - 5) * 6) : a.c.Distance <= 15 ? a.c.DeliveryCharge - 50 : (60 + ((a.c.Distance - 15) * 8))))) + i.Sum(a => a.c.TipsAmount) : i.Sum(a => a.c.DeliveryCharge) + i.Sum(a => a.c.TipsAmount),
        ...
        public double GetPaidAmount(DateTime dateEncoded, string phoneNo)
        {
           ...
           .Select(i => new
           {
               ...
               amount = i.Any() ? i.Sum(a => ((a.c.Distance <= 5) ? 20 : 20 + (a.c.IsPickupDrop == false ? ((a.c.Distance - 5) * 6) : a.c.Distance <= 15 ? a.c.DeliveryCharge - 50 : (60 + ((a.c.Distance - 15) * 8))))) + i.Sum(a => a.c.TipsAmount) : 0
           }).FirstOrDefault();
            if (list != null)
