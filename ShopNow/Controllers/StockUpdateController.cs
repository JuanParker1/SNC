...
namespace ShopNow.Controllers
{
    public class StockUpdateController : Controller
    {
        ...
        public ActionResult StockMaintenance(StockViewModel model)
        {
            var user = ((ShopNow.Helpers.Sessions.User)Session["USER"]);
            ViewBag.Name = user.Name;
            try
            {
                ...
                {
                    string path = Server.MapPath("~/Content/ExcelUpload/" + model.upload.FileName);
                    model.upload.SaveAs(path);
                    if (model.upload != null && model.upload.ContentLength > 0)
                    {
                        string extension = System.IO.Path.GetExtension(model.upload.FileName).ToLower();
                        string Name = model.upload.FileName.Replace(extension, Convert.ToString(shopmodel.Id));
                        Name = Name + extension;
                        //string connString = "";
                        ...
                        if (validFileTypes.Contains(extension))
                        {
                            ...
                            Stream stream = model.upload.InputStream;
                            ...
                            model.Filename = model.upload.FileName;
                            return View(model);
                        }
                        else
                        {
                ...
                else
                {
                    ...
                    string Name = model.Filename.Replace(extension, Convert.ToString(shopmodel.Id));
                    Name = Name + extension;
                    string connString = "";
                    string[] validFileTypes = { ".xls", ".xlsx", ".csv" };
