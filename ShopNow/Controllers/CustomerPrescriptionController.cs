...
namespace ShopNow.Controllers
{
    public class CustomerPrescriptionController : Controller
    {
        ...
        public async Task<JsonResult> GetShopProductSelect2(int shopid, string q = "")
        {
            if (shopid == 322)
            {
                var model = await db.Products.Where(a => a.ShopId == shopid && a.Status == 0 && a.Qty >0)
                   ...
                   // .Join(db.Categories, p => p.p.CategoryId, c => c.Id, (p, c) => new { p, c })
                    ...
                    .Select(i => new
                    {
                        ...
                        categoryname ="",
                        imagepath = "https://s3.ap-south-1.amazonaws.com/shopnowchat.com/Medium/" + i.m.ImagePath1,
                        itemid = i.p.ItemId,
                        quantity = i.p.Qty,
